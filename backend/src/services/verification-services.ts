import { verifications, type NewVerification, type UpdateVerification } from "@/schema/verification";
...
export const verifyVerification = async (data: UpdateVerification) => {
  if (data.id) {
    ...
    }).from(verifications).where(eq(verifications.id, data.id));
    if (verification.length > 0 && verification[0]?.expiresAt) {
      ...
      if (!isExpired) {
        return await db.update(verifications).set(data).where(eq(verifications.id, data.id)).returning({
          id: verifications.id,
